...
import fr.craftyourliferp.data.BlackMarketData;
...
public class ItemStackProbability {
    ...
    public boolean addItem(String id, int quantity, float priceInEuro, int dropChance, String displayName)
    {
    ...
    public void writeToNBT(String key, NBTTagCompound compound)
	{
		...
		for(MarketItem marketItem : items.keySet())
		{
			...
			tag.setFloat("Price", marketItem.getPriceInEuro());
	...
	public void readFromNBT(String key, NBTTagCompound compound)
	{
		...
		for(int i = 0; i < ItemStackProbabilityList.tagCount(); i++)
		{
			...
			MarketItem marketItem = new MarketItem(ItemStack.loadItemStackFromNBT(tag),tag.getFloat("Price"));
