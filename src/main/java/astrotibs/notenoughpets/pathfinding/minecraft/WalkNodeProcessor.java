        ...
        else
        {
            ...
            if (pathpoint != null)
            {
                ...
                for (k = 0; y > 0; pathpoint = this.openPoint(x, y, z))
                {
                    ...
                    if (j++ >= entityIn.getMaxSafePointTries()) // This is called getMaxFallHeight() in 1.8?
                    {
                        return null;
                    }
        ...
        for (int i = x; i < x + sizeX; ++i)
        {
            for (int j = y; j < y + sizeY; ++j)
            {
                for (int k = z; k < z + sizeZ; ++k)
                {
                    ...
                    Block block = blockaccessIn.getBlock(blockpos$mutableblockpos.getX(), blockpos$mutableblockpos.getY(), blockpos$mutableblockpos.getZ());//.getBlockState(blockpos$mutableblockpos).getBlock();

                    if (block.getMaterial() != Material.air)
                    {
                        if (block != Blocks.trapdoor)// && block != Blocks.iron_trapdoor) //Iron Trapdoor introduced in 1.8
                        {
