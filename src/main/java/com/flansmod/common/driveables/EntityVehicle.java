...
public class EntityVehicle extends EntityDriveable implements IExplodeable
{
	...
	public List<Integer> sendedRadioPacketToEntity = new ArrayList();
	...
	public EntityVehicle(World world, double x, double y, double z, EntityPlayer p, VehicleType type, DriveableData data, ItemStack is)
	{
		super(world, type, data);

    	attribuatedItemStack = is;
		...
		data.lastEntityId = getEntityId();
		data.spawnedTime = new Timestamp(System.currentTimeMillis());
	    data.placerName = this.placerName;
	}
	...
	public boolean interactFirst(EntityPlayer entityplayer)
    {
		...
		if(currentItem != null && currentItem.getItem() instanceof ItemTool && ((ItemTool)currentItem.getItem()).type.healDriveables) return true;
	...
	public boolean pressKey(int key, EntityPlayer player)
	{
    	...
    	if(worldObj.isRemote && (key == 6 || key == 8 || key == 9 || key == 19))
    	{
		...
		switch(key)
		{
			...
			case 1 : //Decelerate : Decrease the throttle, down to -1, or 0 if the vehicle cannot reverse
			{
				...
				if(throttle < 0F && type.maxNegativeThrottle == 0F) throttle = 0F;
			...
			case 4 :
			{
				if(throttle > 0.3F || throttle < -0.3F)
				{
					for(EntityWheel wheel:wheels)
					{
						...
						if(wheel.ID == 0 || wheel.ID == 1) FlansMod.getPacketHandler().sendToAllAround(new PacketParticle("explode",wheel.posX, wheel.posY, wheel.posZ, 0,0,0), posX, posY, posZ, 150, dimension);
					}
				}
				
				throttle *= 0.8F;
				
				if(throttle < 0.001F && throttle > 0) throttle = 0F;
			...
			case 13 :
			{
				if(seats[0].targetPitch < -seats[0].seatInfo.minPitch) seats[0].targetPitch += seats[0].seatInfo.aimingSpeed.y;
				return true;
			}
			case 14 :
			{
				...
				{
					toggleTimer = 10;
					if(!getDriveableData().varDoor)
					{
						seats[0].OpenDoor(player,true);
					}
					else
					{
						seats[0].OpenDoor(player,false);
					}
				}
				return true;
			}
			case 15 :
			{
				if(seats[0].targetPitch > -seats[0].seatInfo.maxPitch) seats[0].targetPitch -= seats[0].seatInfo.aimingSpeed.y;
				return true;
			}
            case 16 : 
            {
            	FlansMod.getPacketHandler().sendToServer(new PacketCYLRPSound(this.getDriveableType().klaxonSound));
            	return true;
            }
            case 17 :
            {
            	PacketRadio packet = new PacketRadio((byte)0);
            	packet.vehicleId = this.getEntityId();
            ...
            /*case 19:
            {
            	...
            	{
            		toggleTimer = 10;
            		if(getVehicleType().hasSiren)
            		{
            			activateSiren(!sirenActivated, player);
            		}
            		else
            		{
            			player.addChatComponentMessage(new ChatComponentText("§cCe véhicule n'a pas de siren!"));
            		}
            	}
            }*/
		}
	...
	public void onUpdate()
    {
    	...
    	{	    	
			...
			if(this.getDriveableData() != null && worldObj.getPlayerEntityByName(getDriveableData().placerName) == null)
			{
	    	...
	    	{
	    		WorldData world = WorldData.get(worldObj);
	    		if(world.thieftAreaSelector.entityInsideSelection(wheels[0]))
	    		{
	    			if(seats[0].riddenByEntity != null && !seats[0].riddenByEntity.getCommandSenderName().equals(getDriveableData().ownerName))
	    			{
	    					
	    				if(seats[0].riddenByEntity instanceof EntityPlayer)
	    				{	
			
		    				EntityPlayer thief = (EntityPlayer) seats[0].riddenByEntity;			
		    					
		    				System.out.println("/eco give " + thief.getCommandSenderName() + " 1500");
		    				thief.addChatMessage(new ChatComponentText("§2Vous venez de gagner §a1500€ §2!"));
		    		    					
		    				if(driveableData.ownerName != null)
		    				{
			    				EntityPlayer player = worldObj.getPlayerEntityByName(driveableData.ownerName);
			    				if(player != null && attribuatedItemStack != null && attribuatedItemStack.getItem() != null)
			    				{
			    					ServerUtils.sendChatMessage(player, "§cVotre véhicule a été volé!");
									ExtendedPlayer extendedP = ExtendedPlayer.get(player);
									if(extendedP != null) extendedP.stolenVehiclesId.add(Item.getIdFromItem(attribuatedItemStack.getItem()));
			    					world.driveablesManager.removeDriveablesFromSameType(this, player);
			    				}
			    				else
			    				{
			    					setDead();
		    				...
		    				{
		    					world.driveablesManager.removeDriveablesFromSameType(this, placerName);
		    				}
    	...
    	{
    		throttle = MathsUtils.Lerp(throttle, 0, 0.01f);
    		
    		if(CraftYourLifeRPMod.radioHandler.getCurrentRadio() != null) RadioGui.sendRadioToVehicleEntities(CraftYourLifeRPMod.radioHandler.getCurrentRadio().getIndex(), this);
    	
    		List<Integer> toRemove = new ArrayList();
    		for(Integer id : sendedRadioPacketToEntity)
    		{
    			...
    			else if(player.ridingEntity != null)
    			{
    				...
    				{
    					toRemove.add(player.getEntityId());
    				}
    			}
    			else
    			{
    		...
    		sendedRadioPacketToEntity.removeAll(toRemove);
    	}
        ...
        {
        	canFire = this.getDriveableData().varDoor;
        }
		...
		if(!worldObj.isRemote && seats[0].riddenByEntity != null) ticksSinceUsed = 0;
		...
		if(type.setPlayerInvisible && !this.worldObj.isRemote && seats[0].riddenByEntity != null) seats[0].riddenByEntity.setInvisible(true);

		if(this.ticksFlareUsing<= 0) deployedSmoke = false;

		if(this.ticksFlareUsing > 0) this.ticksFlareUsing--;
		
		if(this.flareDelay > 0) this.flareDelay--;


		if(shellDelay > 0) shellDelay--;
		
		if(gunDelay > 0) gunDelay--;
		
		if(toggleTimer > 0) toggleTimer--;
		
		if(toggleTimerZoom > 0) toggleTimerZoom--;
		
		if(soundPosition > 0) soundPosition--;
		
		if(idlePosition > 0) idlePosition--;
		
		if(driftPosition > 0) driftPosition--;
		...
		if(!this.getDriveableData().varDoor)
		{
		...
		for(EntityWheel wheel : wheels)
		{
			...
			boolean canThrustCreatively = !FlansModSettings.instance.vehiclesNeedFuel || (seats != null && seats[0] != null && seats[0].riddenByEntity instanceof EntityPlayer && ((EntityPlayer)seats[0].riddenByEntity).capabilities.isCreativeMode);
			...
			if(canThrustCreatively || data.fuelInTank > data.engine.fuelConsumption * throttle)
			{
				...
				else
				{
					...
					if(!stopCalcul)
					{
						speed = (float) wheel.getSpeedXZ() * 140;
						stopCalcul = true;
					}
		...
		{
			FlansMod.getPacketHandler().sendToAllAround(new PacketDriveableControl(this), posX, posY, posZ, FlansMod.driveableUpdateRange, dimension);
			//if(type.IT1)
