...
import com.flansmod.common.network.PacketEjectPlayers;
...
public abstract class EntityDriveable extends Entity implements IControllable, IExplodeable, IEntityAdditionalSpawnData
{
	...
	public ItemStack attribuatedItemStack;
    ...
    protected void writeEntityToNBT(NBTTagCompound tag)
    {
		...
		if(key != null) tag.setString("key", key);
	...
	private void shootEach(DriveableType type, ShootPoint shootPoint, int currentGun, boolean secondary, EnumWeaponType weaponType)
	{
		...
		if(shootPoint.rootPos instanceof PilotGun)
		{
			...
			if(type.rangingGun) shellSpeed = type.bulletSpeed;
			...
			if(gunType != null && bulletItemStack != null && bulletItemStack.getItem() instanceof ItemShootable && FlansModSettings.instance.bulletsEnabled)
			{
		...
		else //One of the other modes
		{
			...
			case BOMB :
			{
				if(FlansModSettings.instance.bombsEnabled)
				{
					int slot = -1;
			...
			case SHELL :
			{
				...
				if(FlansModSettings.instance.shellsEnabled)
				{
					int slot = -1;
    ...
    public void onUpdate()
    {
		...
		if(!worldObj.isRemote)
		{
			...
			if(worldObj.getTotalWorldTime() % 20 == 0)
			{
	    		...
	    		{
	    			setFire(40);
	    			attackPart(EnumDriveablePart.core, DamageSource.inFire,(float)MathHelper.getRandomDoubleInRange(worldObj.rand, 0, 0.000036D));	    			
	    		}
			}
		}
		...
		//Actual harvesting
		if(type.harvestBlocks && type.harvestBoxSize != null && type.harvestBoxPos != null && FlansModSettings.instance.driveablesBreakBlocks)
		{
	...
	public void checkForCollisions()
	{
		...
		for(DriveablePosition p : getDriveableType().collisionPoints)
		{
			...
			if(hit != null && hit.typeOfHit == MovingObjectType.BLOCK)
			{
				...
				//Attack the part
				if(!attackPart(p.part, DamageSource.inWall, damage) && FlansModSettings.instance.driveablesBreakBlocks)
				{
	...
	public void moveRiders(Entity rider)
	{
		...
		if(getDriveableType().collisionDamageEnable && !test.isOnTop)
		{
			if(throttle > getDriveableType().collisionDamageThrottle)
			{
				...
				if(rider instanceof EntityPlayerMP && seats[0].riddenByEntity instanceof EntityPlayer)
				{
...
//						((EntityPlayer)seats[i].riddenByEntity).addPotionEffect(new PotionEffect(Potion.harm.id, 10, 5));
		        ...
		        EntityPlayer ownerP = worldObj.getPlayerEntityByName(getDriveableData().ownerName);
		        if(ownerP != null)
		        {
				...
				WorldUtils.createFlameArroundEntity(worldObj, this);
			}
			setDead();
	...
	public String getProprio() {
		return this.getDriveableData().ownerName;
	}

	public void setproprio(String proprio) {
		this.getDriveableData().ownerName = proprio;
	}


	public boolean getDoor() {
		return this.getDriveableData().varDoor;
	}

	public void setdoor(boolean door) {
		this.getDriveableData().varDoor = door;
	}

	public int getTimer() {
		return this.getDriveableData().timerToSteal;
	}

	public void setTimer(int timer) {
		this.getDriveableData().timerToSteal = timer;
	}
	
	public void ejectPlayersFromVehicle() {
		if(worldObj.isRemote) {
			DriveableData d = this.getDriveableData();
			for(EntitySeat e : seats) {
				if(e.riddenByEntity != null && e.riddenByEntity.getCommandSenderName() != getDriveableData().ownerName && e.riddenByEntity.getCommandSenderName() != Minecraft.getMinecraft().thePlayer.getCommandSenderName())
				{
					EntityPlayer p = (EntityPlayer)e.riddenByEntity;
					p.closeScreen();
					FlansMod.getPacketHandler().sendToServer(new PacketEjectPlayers(p.getEntityId()));
				}
			}
			Minecraft.getMinecraft().thePlayer.addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "Tous les joueurs on \u00E9tait exclue du v\u00E9hicule"));
		}
	}
	
	public String getDriveableName()
	{
		if(getDriveableType() == null && this.getDriveableData().type == null) return "Unkown Vehicle";
		
		return getDriveableType().name == null ? getDriveableType().shortName == null ? getDriveableData().type : getDriveableType().shortName :  getDriveableType().name;
	}
