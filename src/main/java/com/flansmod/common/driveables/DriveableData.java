...
public class DriveableData implements IInventory
{
	...
	public boolean varDoor = true;
	public int timerToSteal;
	public int lastEntityId;
	
	public String placerName;
	
	public Timestamp spawnedTime;
	...
	public void readFromNBT(NBTTagCompound tag)
    {
		...
		for(int i = 0; i < numGuns; i++) ammo[i] = ItemStack.loadItemStackFromNBT(tag.getCompoundTag("Ammo " + i));
		
		for(int i = 0; i < numBombs; i++) bombs[i] = ItemStack.loadItemStackFromNBT(tag.getCompoundTag("Bombs " + i));

		for(int i = 0; i < numMissiles; i++) missiles[i] = ItemStack.loadItemStackFromNBT(tag.getCompoundTag("Missiles " + i));

 		for(int i = 0; i < numCargo; i++) cargo[i] = ItemStack.loadItemStackFromNBT(tag.getCompoundTag("Cargo " + i));
		...
		this.timerToSteal = tag.getShort("TimerToSteal");
		this.lastEntityId = tag.getInteger("LastEntityId");
    ...
    public void writeToNBT(NBTTagCompound tag)
    {
		...
		{
			if(ammo[i] != null)
				tag.removeTag("Ammo " + i);
		}
		for(int i = 0; i < bombs.length; i++)
		{
			if(bombs[i] != null)
				tag.removeTag("Bombs " + i);
		}
		for(int i = 0; i < missiles.length; i++)
		{
			if(missiles[i] != null)
				tag.removeTag("Missiles " + i);
		}
		...
		{
			tag.setString("OwnerName", "");
		}
		
        tag.setInteger("TimerToSteal", this.timerToSteal);
        tag.setInteger("LastEntityId", this.lastEntityId);
