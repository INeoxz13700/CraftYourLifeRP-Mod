...
public class PacketDriveableGUI extends PacketBase
{
	...
	public void handleServerSide(EntityPlayerMP playerEntity) 
	{
		if(playerEntity.ridingEntity != null && playerEntity.ridingEntity instanceof EntitySeat)
		{
			...
			if(d.seats[0].riddenByEntity != null)
			{
				if(!d.getDriveableData().placerName.equalsIgnoreCase(d.seats[0].riddenByEntity.getCommandSenderName()))
				{
					if(d.worldObj.getPlayerEntityByName(d.getDriveableData().ownerName) == null)
					{
						EntityPlayer player = (EntityPlayer)d.seats[0].riddenByEntity;
			        	player.closeScreen();
						player.addChatComponentMessage(new ChatComponentText("§cLe propriétaire du véhicule n'est pas connecté"));
						return;
					}
				}
			}
