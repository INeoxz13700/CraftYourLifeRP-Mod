...
public class EntityGrenade extends EntityShootable implements IEntityAdditionalSpawnData
{
	...
	public void onUpdate()
	{
		...
		if(!stuck && !type.stickToThrower)
		{
			...
			if(hit != null && hit.typeOfHit == MovingObjectType.BLOCK)
			{
				...
				else if(type.breaksGlass && mat == Material.glass && FlansModSettings.instance.canBreakGlass)
				{
	...
	public void detonate()
	{
		...
		if(!worldObj.isRemote && type.explosionRadius > 0.1F)
		{
	        ...
	        {
	        	FlansModExplosion explosion = new FlansModExplosion(worldObj, this, (EntityPlayer)thrower, type, posX, posY, posZ,
		        	type.explosionRadius, FlansModSettings.instance.explosions && type.explosionBreaksBlocks,
	        ...
	        {
	        	worldObj.createExplosion(this, posX, posY, posZ, type.explosionRadius,FlansModSettings.instance.explosions && type.explosionBreaksBlocks);
	        }
		}
		...
		if(!worldObj.isRemote && type.fireRadius > 0.1F)
		{
			...
			if(!worldObj.isRemote)
			{
				...
				{
					WorldData data = WorldData.get(worldObj);
					
					if(worldObj.getTotalWorldTime() - data.lastFireMolotovTime > 20 * 60 * 30)
					{
						data.setLastFireTime(worldObj.getTotalWorldTime(), 3);
						CraftYourLifeRPMod.fireHandler.startFireFromCoordinates(worldObj, (int)posX, (int)posY, (int)posZ, "grenade molotov",70);
					}
				}
			}
		}
